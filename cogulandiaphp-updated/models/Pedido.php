...
class Pedido {
    ...
    public static function criar($cliente_id, $produto_id, $quantidade) {
        ...
        if (!Produto::produtoExiste($produto_id)) {
    
            header('Location: ../views/pedidos/createpedido.php?erro=produto_nao_existe');
            exit; // Encerre a execução do script após o redirecionamento
        }
    
        if (!Cliente::clienteExiste($cliente_id)) {
    
            header('Location: ../views/pedidos/createpedido.php?erro=cliente_nao_existe');
            exit; // Encerre a execução do script após o redirecionamento
        }
